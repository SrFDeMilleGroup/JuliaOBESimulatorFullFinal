...
saveDataFolderTag = string("GoldBlueHigherResD1MOTDetF1_",detF1,"DetF2_",detF2,"s0F1_",s0F1,"s0F2_",s0F2,"bGrad",bGradToUse); #If you want to put anything additional in "savefoldername" to tag it, see variable folderString after lasers are declared.
addHeaders=1;

#3) Non Laser Detuning/Pol Simulation Variables (B-field, beam-waist etc.)
bGradReal = parse(Float64,bGradToUse);# in units Gauss/cm.  if "Static", this becomes the static field, in Gauss\
bGrad = (1 / kA * 1e2) * bGradReal;
numTrialsPerValueSet = 60;#number of trials per set of values (displacementsInMM,userSpeeds,longSpeeds)
...
=#
#displacementsInMM = [0.5,1,2,3,4,5];
displacementsInMM = [.02,.05,.1,.2,.4,.6,.8,1];
#userSpeeds = [-3,-1,-.3,.3,1,3]
#blueMOT
#userSpeeds = [-5,-4,-3,-2.5,-2,-1.5,-1,-.5,-.3,-0.1,-0.05,-.03,-0.02,-0.01,.01,.02,.03,0.05,0.1,.3,.5,1,1.5,2,2.5,3,4,5]
userSpeeds = [-.3,-.2,-.1,-.05,-.02,-.01,-.006,.006,.01,.02,.05,.1,.2,.3];
...
=#
s0 = [parse(Float64,s0F2),parse(Float64,s0F1)].*1.0;
laserEnergy = [9.2+parse(Float64,detF2),9.2+parse(Float64,detF1)];
polSign = -1 .* [Int(1*sign(parse(Float64,detF2))),1];
whichJ = ["1/2","1/2"];
...
if saveData==1
    folderString = string(@__DIR__,"/saveDataRandPhaseGoldBlueRevF1RightPol/",saveDataFolderTag)
    mkpath(folderString)
end
