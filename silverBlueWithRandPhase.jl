...
saveDataFolderTag = string("DetRelFPrime1_",detToUse,"CarInt",s0ToUse,"R01",R01ToUse); #If you want to put anything additional in "savefoldername" to tag it, see variable folderString after lasers are declared.
addHeaders=1;
...
numTrialsPerValueSet = 20;#number of trials per set of values (displacementsInMM,userSpeeds,longSpeeds)
...
displacementsInMM = [.02,.05,.1,.2,.3,.5,.7,1.0];
userSpeeds = [-1.,-.7,-.5,-.4,-.2,-.1,-.05,-.03,-.02,-.01,.01,.02,.03,.05,.1,.2,.4,.5,.7,1.];

#laser parameters, redMOT
#=
s0 = [.8,.1];
laserEnergy = [-3.0,0.];
polSign = [-1,-1];
...
=#
#laser parameters, blueMOT
s0Car = parse(Float64,s0ToUse);
R01 = parse(Float64,R01ToUse);
s0 = [s0Car*(1/(1+R01)),s0Car*(R01/(1+R01))];
laserEnergyToUse = parse(Float64,detToUse)-2.4;
laserEnergy = [laserEnergyToUse,laserEnergyToUse];
polSign = [-1,-1];
whichJ = ["1/2","1/2"];
whichFGround = ["FHigh","FLow"];
waists = [3,3] .* 1e-3 .* kA;#in normalized units
...
if saveData==1
    folderString = string(@__DIR__,"/saveDataSilverBlueRandPhase/",saveDataFolderTag,"bGrad", bGradReal)
    mkpath(folderString)
end
