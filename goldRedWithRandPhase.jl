...
saveDataFolderTag = string("GoldRedHighResD1MOTDetF1_",detF1,"DetF2_",detF2,"s0F1_",s0F1,"s0F2_",s0F2,"bGrad",bGradToUse); #If you want to put anything additional in "savefoldername" to tag it, see variable folderString after lasers are declared.
addHeaders=1;
...
numTrialsPerValueSet = 40;#number of trials per set of values (displacementsInMM,userSpeeds,longSpeeds)
...
if saveData==1
    folderString = string(@__DIR__,"/saveDataGoldRedRandPhaseRevF2MoreTrials/",saveDataFolderTag)
    mkpath(folderString)
end
